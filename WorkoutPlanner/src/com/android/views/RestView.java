...
public class RestView extends AElementView {
	@Override
	public int getViewType() {
		return elementTypes.REST_INDEX.ordinal();
	}

	@Override
	public View getView(LayoutInflater inflater, ViewGroup parent, View convertView, int position) {
		final PreviewRestHolder holder;
		...
		if (convertView == null) {
			...
			holder = (PreviewRestHolder) convertView.getTag();
		}
	...
	public RestView(Rest element, Context context) {
		super(context);
		mElement = element;
	}

	public RestView(RestView toClone) {
		super(toClone.mContext);
		mElement = new Rest((Rest) toClone.mElement);
	}
}
